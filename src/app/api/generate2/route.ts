...
export async function POST(req: NextRequest) {
  ...
  const garmentTypeMap: Record<string, "upper" | "lower" | "overall"> = {
    tops: "upper",
    bottoms: "lower",
    "one-pieces": "overall",
  };
  ...
  const vtonResults = await Promise.all([
    fal.subscribe("fal-ai/cat-vton", {
      input: {
        ...
        cloth_type: garmentTypeMap[garmentType] || "upper",
      },
    }),
    fal.subscribe("fal-ai/leffa/virtual-tryon", {
      input: {
        human_image_url: modelUrl,
        garment_image_url: garmentUrl,
      },
  ...
  if (!isUpscale) {
    return NextResponse.json({
      output: [vtonResults[0].data.image.url, vtonResults[1].data.image.url],
    });
  }
  ...
  const upscaleResults = await Promise.all([
    upscale(vtonResults[0].data.image.url),
